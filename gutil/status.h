...
#ifndef GOOGLE_NETKAT_GUTIL_STATUS_H
#define GOOGLE_NETKAT_GUTIL_STATUS_H
...
#define RETURN_IF_ERROR(expr)                     \
  ...
  return netkat::StatusBuilder(std::move(status))
...
#define __ASSIGN_OR_RETURN_STREAM(dest, expr, stream)      \
  auto __ASSIGN_OR_RETURN_VAL(__LINE__) = expr;            \
  if (!__ASSIGN_OR_RETURN_VAL(__LINE__).ok()) {            \
    return ::netkat::status_internal::StatusBuilderHolder( \
...
#define RET_CHECK(cond) \
  while (!(cond))       \
  return netkat::InternalErrorBuilder() << "(" << #cond << ") failed"

#endif  // GOOGLE_NETKAT_GUTIL_STATUS_H_
