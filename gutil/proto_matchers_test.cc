...
TEST(ProtoMatcher, DescribeEqualsProto) {
  auto matcher = EqualsProto(netkat::ParseProtoOrDie<TestMessage>(R"pb(
    int_field: 123
    string_field: "foo"
    bool_field: true
  )pb"));
  ...
  EXPECT_EQ(testing::DescribeMatcher<TestMessage>(matcher),
            R"(is equal to netkat.TestMessage <
int_field: 123
string_field: "foo"
bool_field: true
>)");
  EXPECT_EQ(testing::DescribeMatcher<TestMessage>(Not(matcher)),
            R"(is not equal to netkat.TestMessage <
int_field: 123
string_field: "foo"
bool_field: true
>)");
...
TEST(HasOneofCaseTest, NotHasOneofCase) {
  EXPECT_THAT(netkat::ParseProtoOrDie<TestMessageWithOneof>(R"pb()pb"),
              Not(HasOneofCase<TestMessageWithOneof>(
                  "foo", TestMessageWithOneof::kStringFoo)));
  EXPECT_THAT(netkat::ParseProtoOrDie<TestMessageWithOneof>(R"pb(
                int_foo: 42
              )pb"),
              Not(HasOneofCase<TestMessageWithOneof>(
                  "foo", TestMessageWithOneof::kStringFoo)));
...
TEST(HasOneofCaseTest, DoesHaveOneofCase) {
  EXPECT_THAT(netkat::ParseProtoOrDie<TestMessageWithOneof>(R"pb(
              ...
              HasOneofCase<TestMessageWithOneof>(
                  "foo", TestMessageWithOneof::kStringFoo));
  EXPECT_THAT(
      netkat::ParseProtoOrDie<TestMessageWithOneof>(R"pb(
        int_foo: 42
      )pb"),
...
TEST(HasOneofCaseTest, Description) {
  ...
  EXPECT_EQ(testing::DescribeMatcher<TestMessageWithOneof>(matcher),
            "is a `netkat.TestMessageWithOneof` protobuf message whose oneof "
