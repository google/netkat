...
TEST(SaveProtoToFile, SavesProtoToFileTruncatesFileOnOverwrite) {
  ...
  ASSERT_OK_AND_ASSIGN(netkat::TestMessage proto,
  ...
  netkat::TestMessage read_proto;
  ASSERT_OK(netkat::ReadProtoFromFile(proto_save_path, &read_proto));
  EXPECT_THAT(read_proto, EqualsProto(proto));

  netkat::TestMessage empty_proto;
  // Overite the saved file with empty proto.
  ASSERT_OK(SaveProtoToFile(proto_save_path, empty_proto));

  netkat::TestMessage read_empty_proto;
  ASSERT_OK(netkat::ReadProtoFromFile(proto_save_path, &read_empty_proto));
