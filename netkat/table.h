...
//
// Any additional capabilities and expectations of each rule may be added via
// `TableConstraint`s.
class NetkatTable {
 public:
  // Returns a NetKAT Table as described as above.
  //
  ...
  // match/policy of the given priority space, or null if this is the first
  // entry, and the priority. It is required that each constraint return a
  // status reflecting OK if the rules conform or an error detailing the
  // violation.
  //
  // Note that `current_policy` will be the total policy of the table. E.g. for
  // some (p1, p1_action), (p2, p2_action) this will be:
  //
