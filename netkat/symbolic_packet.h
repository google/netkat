...
class SymbolicPacketManager {
 public:
  ...
  bool Contains(SymbolicPacket symbolic_packet, Packet concrete_packet) const;
 ...
 private:
  ...
  struct DecisionNode {
    ...
    // Logically this is a value -> branch map, but we store it as a fized-size
    ...
    // * We don't need fast lookups, so we can avoid the overhead of
    //   lookup-optimized data structures like hash maps.
    //
    // INVARIANTS:
    ...
    // 3. The pairs are ordered by strictly increasing value. No two branches
    //    have the same value.
  ...
  // `And`, `Or`, `Not`). The class also avoids expensive reallocations.
