...
PacketTransformerHandle CounterExample::GetLeftDiffRightTransformer() const {
  return left_diff_right_transformer_;
}
PacketTransformerHandle CounterExample::GetRightDiffLeftTransformer() const {
  return right_diff_left_transformer_;
}

PacketSetHandle CounterExample::GetInputFromLeftDiffRightPolicies() const {
  return input_from_left_diff_right_policies_;
}
PacketSetHandle CounterExample::GetInputFromRightDiffLeftPolicies() const {
  return input_from_right_diff_left_policies_;
}

std::string CounterExample::GetInputFromLeftDiffRightPoliciesAsDot() const {
  CHECK(packet_transformer_manager_ != nullptr);
  return packet_transformer_manager_->GetPacketSetManager().ToDot(
      input_from_left_diff_right_policies_);
}

std::string CounterExample::GetInputFromRightDiffLeftPoliciesAsDot() const {
  CHECK(packet_transformer_manager_ != nullptr);
  return packet_transformer_manager_->GetPacketSetManager().ToDot(
...
void AbslStringify(Sink& sink, const CounterExample& counter_example) {
  absl::Format(
      ...
      counter_example.GetInputFromLeftDiffRightPoliciesAsDot(),
      counter_example.GetInputFromRightDiffLeftPoliciesAsDot());
  if (!counter_example
...
std::ostream& operator<<(
  ...
  if (success_or_counter_example.IsSuccess()) {
    ...
    return os << success_or_counter_example.GetCounterExampleOrDie();
  }
}
