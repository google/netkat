...
// function of type Packet -> Packet Set.
//
...
class SymbolicPacketTransformerManager {
 public:
  ...
  // The symbolic packet transformer representing the Deny policy.
  SymbolicPacketTransformer Deny() const;

  // The symbolic packet transformer representing the Accept policy.
  SymbolicPacketTransformer Accept() const;

  // Creates a symbolic packet transformer that forwards the set of packets
  // represented by the given `symbolic_packet` belonging to `manager`,
  // unchanged and drops any other packets.
  SymbolicPacketTransformer OfSymbolicPacket(
      const SymbolicPacketManager& manager, SymbolicPacket symbolic_packet);

  // Returns the transformer that forwards packets unchanged if their `field` is
  // equal to `value`, and drops them otherwise.
  SymbolicPacketTransformer Filter(const PredicateProto& predicate);

  // Returns the transformer that modifies packets such that their `field` is
  // equal to `value`, then forwards them.
  ...
  // TODO(dilo): There are many additional operations supported by this data
  // structure, but not currently implemented. Add them as needed. Examples:
  // * intersection - which output packets are possible in both policies?
  // * difference - which output packets are possible in the first policy, but
  //                not the second?
  // * symmetric difference - which output packets are possible in either
  //                          policy, but not both?
