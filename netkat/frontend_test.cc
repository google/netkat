...
// test on.
fuzztest::Domain<Predicate> ArbitraryPredicateDomain() {
...
FUZZ_TEST(FrontEndTest, NegateToProtoIsCorrect)
    .WithDomains(ArbitraryPredicateDomain());
...
FUZZ_TEST(FrontEndTest, AndToProtoIsCorrect)
    .WithDomains(/*lhs=*/ArbitraryPredicateDomain(),
                 /*rhs=*/ArbitraryPredicateDomain());
...
FUZZ_TEST(FrontEndTest, OperationOrderIsPreserved)
    .WithDomains(/*a=*/ArbitraryPredicateDomain(),
                 /*b=*/ArbitraryPredicateDomain(),
                 /*c=*/ArbitraryPredicateDomain());
...
fuzztest::Domain<Policy> ArbitraryPolicyDomain() {
  return OneOf(Map(Filter, ArbitraryPredicateDomain()),
...
FUZZ_TEST(FrontEndTest, FilteredPredicateToProtoIsCorrect)
    .WithDomains(/*predicate=*/ArbitraryPredicateDomain());
...
FUZZ_TEST(FrontEndTest, IterateToProtoIsCorrect)
    .WithDomains(/*policy=*/ArbitraryPolicyDomain());
...
FUZZ_TEST(FrontEndTest, SequencePreservesOrder)
    .WithDomains(
        /*policies=*/ContainerOf<std::vector<Policy>>(ArbitraryPolicyDomain())
            .WithMinSize(2));

void SequenceNArgsIsOk(Policy a, Policy b, Policy c) {
  EXPECT_THAT(Sequence(a, b, c).ToProto(),
              EqualsProto(SequenceProto(SequenceProto(a.ToProto(), b.ToProto()),
                                        c.ToProto())));
}
FUZZ_TEST(FrontEndTest, SequenceNArgsIsOk)
    .WithDomains(
        /*a=*/ArbitraryPolicyDomain(),
        /*b=*/ArbitraryPolicyDomain(),
        /*c=*/ArbitraryPolicyDomain());
...
void UnionNArgsIsOk(Policy a, Policy b, Policy c) {
  EXPECT_THAT(Union(a, b, c).ToProto(),
              EqualsProto(UnionProto(UnionProto(a.ToProto(), b.ToProto()),
                                     c.ToProto())));
}
FUZZ_TEST(FrontEndTest, UnionNArgsIsOk)
    .WithDomains(
        /*a=*/ArbitraryPolicyDomain(),
        /*b=*/ArbitraryPolicyDomain(),
        /*c=*/ArbitraryPolicyDomain());
