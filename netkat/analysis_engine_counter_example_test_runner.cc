...
void main() {
  ...
  for (const TestCase& test_case : TestCases()) {
    SuccessOrCounterExample counter_example = analysis_engine.CheckEquivalent(
        test_case.left_policy, test_case.right_policy);
    ...
    std::cout << kCounterExampleHeader << counter_example;
  }
}
