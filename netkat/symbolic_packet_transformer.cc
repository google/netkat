...
absl::flat_hash_set<Packet> SymbolicPacketTransformerManager::Run(
  ...
  if (it != concrete_packet.end()) {
    ...
    if (auto modification_map =
            node.modify_branch_by_field_match.find(it->second);
        modification_map != node.modify_branch_by_field_match.end()) {
      matched = true;
      for (const auto& [value, branch] : modification_map->second) {
        Packet modified_packet = concrete_packet;
        modified_packet[field] = value;
        // @anthonyroy and @smolkaj, does it even help to move when the method
        // takes a const ref? Probably not? Or perhaps it's even wrong and makes
        // it live less long? Not sure if there's something else clever I can do
        // to avoid creating a bunch of extra packets that only differ in the
        // value of one field?
        result.merge(Run(branch, std::move(modified_packet)));
      }
    }
  }
  ...
  if (!matched) {
    for (const auto& [value, branch] :
      ...
      // branch.
      if (it != concrete_packet.end() && it->second == value) matched = true;
      Packet modified_packet = concrete_packet;
      modified_packet[field] = value;
      result.merge(Run(branch, std::move(modified_packet)));
    }
