...
// Get input packets that generate packets satisfying predicate when run through
// policy."
//
// Semantically, `Pull(policy, predicate)` is a predicate that matches an input
// packet if and only if processing that packet with `policy` yields at least
// one output packet that satisfies `predicate`.
//
