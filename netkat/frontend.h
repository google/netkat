...
// predecence order and, as per the literature, logically behave as expected.
...
// However, this API intentionally exposes and hides certain elements to provide
// some compile time correctness for NetKAT programs.
class Predicate {
 public:
...
class Policy {
 public:
  ...
  // operations definition. We utilize friend association to ensure program
...
// Coerces a `Predicate` into a `Policy`.
//
// This does not semantically change the predicate, rather it simply allows you
// to compose policies of predicates. The reason predicates must be wrapped is
// policy operations are more restrictive.
//
// In general prefer to combine predicates using predicate operations, not
// policy operations.
...
//
// Similar to `Match` except this sets `field` to `new_value`.
...
// semantically different as sequential composition is non-commutative.
//
...
//        Iterate(Sequence(Filter(Match("switch", X)), set_any_port, topology));
